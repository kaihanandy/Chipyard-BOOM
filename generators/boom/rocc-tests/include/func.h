...
////Spike only accept to import your custom function in <file.h>, unaccepted to import <file.c>.
...
//        printf("------------- Features reshaped successfully -------------\n");
//        printf("------------- Start Gemmini tiled_matmul_auto -------------\n");
...
static int find_most_common(int arr[], int size) {
    int count[100] = {0};
    int max_count = 0;
...
static int find_max_channel(int batch_size, int output_width, int out_channels,
                            elem_t out_feature[batch_size][output_width][out_channels]) {
    int max_channel[batch_size];
    for (int i = 0; i < batch_size; i++) {
        int max_value = out_feature[i][0][0];
        int max_ch = 0;
        for (int j = 0; j < output_width; j++) {
            for (int k = 0; k < out_channels; k++) {
                if (out_feature[i][j][k] > max_value) {
                    max_value = out_feature[i][j][k];
                    max_ch = k;
                }
            }
        }
        max_channel[i] = max_ch;
        printf("Window %d (batch size %d): gesture predict %d\n", i + 1, i + 1, max_ch + 1);  // start from 0, add 1
    }

    int most_common_value = find_most_common(max_channel, batch_size);

    printf("\nGesture Predict Answer: %d\n", most_common_value + 1);  // start from 0, add 1
    return most_common_value + 1;
}
